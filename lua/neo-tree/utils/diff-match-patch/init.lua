-- Modified by pynappo - stylua format, lua-ls annotations, no bit-specific things, removed diff/patch/html parts
...
local diff_main, diff_cleanupSemantic, diff_cleanupEfficiency, diff_levenshtein

---The data structure representing a diff is an array of tuples:
---{{DIFF_DELETE, 'Hello'}, {DIFF_INSERT, 'Goodbye'}, {DIFF_EQUAL, ' world.'}}
---which means: delete 'Hello', add 'Goodbye' and keep ' world.'
---@class neotree.lib.diffmatchpatch.patch
---@field [1] neotree.lib.diffmatchpatch.difftype
---@field [2] string

local DIFF_DELETE = -1
local DIFF_INSERT = 1
local DIFF_EQUAL = 0
---@enum neotree.lib.diffmatchpatch.difftype
local difftype = {
...
local _diff_compute, _diff_bisect, _diff_bisectSplit, _diff_halfMatchI, _diff_halfMatch, _diff_cleanupSemanticScore, _diff_cleanupSemanticLossless, _diff_cleanupMerge, _diff_commonPrefix, _diff_commonSuffix, _diff_commonOverlap, _diff_xIndex, _diff_text1, _diff_text2, _diff_toDelta, _diff_fromDelta

---Find the differences between two texts.  Simplifies the problem by stripping
---any common prefix or suffix off the texts before diffing.
---@param text1 string Old string to be diffed.
---@param text2 string New string to be diffed.
---@param opt_checklines boolean? Has no effect in Lua.
---@param opt_deadline number? Optional time when the diff should be complete
---    by.  Used internally for recursive calls.  Users should set DiffTimeout
---    instead.
---@return neotree.lib.diffmatchpatch.patch[]
...
function _diff_fromDelta(text1, delta)
  ...
  if pointer ~= #text1 + 1 then
    error(
      "Delta length (" .. (pointer - 1) .. ") does not equal source text length (" .. #text1 .. ")."
    )
  end
  return diffs
end
